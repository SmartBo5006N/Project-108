...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmark in results.multi_hand_landmarks:
            ...
            #Array to see if finger is folded by true/false    
            finger_fold_status =[]
            for tip in finger_tips:
            #Getting tip position and circle drawing
                x,y = int(lm_list[tip].x*w), int(lm_list[tip].y*h)
                cv2.circle(img, (x,y), 15, (255, 0, 0), cv2.FILLED)

            #Checking if finger is folded or not, and if folded to change color to green
